...
class BTOParser(BianaParser):
    """
    Brenda Tissue OBO Parser Class
    """

    name = "brenda_tissue_obo"
    ...
    def __init__(self):
        ...
        BianaParser.__init__(self, default_db_description = "Brenda Tissue Ontology obo",
                             default_script_name = "BrendaTissueParser.py",
                             default_script_description = "This program fills up tables in database biana related to OBO 1.2 formatted Ontologies")
        self.default_eE_attribute = "bto"
        

    def parse_database(self):
        """
        Method that implements the specific operations of Brenda Tissue obo parser
        """


        # Add a different type of external entity
        self.biana_access.add_valid_external_entity_type( type = "BTOElement" )

        # Add the attributes specific of Brenda Tissue OBO database
        ...
        # Add the possibility to transfer BTO name using BTO term as a key
        self.biana_access._add_transfer_attribute( externalDatabaseID = self.database.get_id(), 
                                                   key_attribute = "BTO",
                                                   transfer_attribute="BTO_name" )

        ontology = Ontology( source_database = self.database, linkedAttribute="BTO", name="BTO", descriptionAttribute="BTO_name" )
	...
	def create_external_entity_from_bto_term(database, term_id, term_name, 
		term_namespace, term_def, term_exact_synonyms, term_related_synonyms, term_broad_synonyms, term_narrow_synonyms, term_alt_id):

	    externalEntity = ExternalEntity( source_database = database, type = "BTOElement" )
	    
	    externalEntity.add_attribute( ExternalEntityAttribute( attribute_identifier = "BTO", value = term_id, type="unique") )

	    if term_name is not None:
		externalEntity.add_attribute( ExternalEntityAttribute( attribute_identifier = "BTO_name", value = term_name, type="unique") )
		...
		if current_synonym is not None:
		    externalEntity.add_attribute( ExternalEntityAttribute( attribute_identifier = "BTO_name",
        ...
        # Quim Aguirre: Adding the alternative BTO ids as "alias" in BTO table
	    for current_alt_id in term_alt_id:
		if current_alt_id is not None:
		    externalEntity.add_attribute( ExternalEntityAttribute( attribute_identifier = "BTO",
        ...
        for line in self.input_file_fd:
            ...
            if re.search("\[Term\]",line) or re.search("\[Typedef\]",line):
                ...
                if term_id is not None:
                    ...
                    # insert previous
		    externalEntity = create_external_entity_from_bto_term(self.database, term_id, term_name, 
            ...
            # Example --> alt_id: BTO:0001414
                ...
                if( re.search("part_of",line) ):
                    temp = re.search("part_of\s+BTO\:(\d+)",line)
                    if temp is not None:
