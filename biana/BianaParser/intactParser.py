...
class Psi_MiFormattedDBParser(BianaParser):
    ...
    def __init__(self):
        ...
        BianaParser.__init__(self, default_db_description = "PSI-MI formatted protein-protein interaction database",
                             default_script_name = "intactParser.py",
	...
	self.file_number = 0
        ...
        ###############################
        # Parse the IntAct FASTA file #
        ###############################

        FastaFile = os.path.join(directoryData, 'intact.fasta')
        if not os.path.isfile(FastaFile):
            sys.stderr.write('We need to have the FASTA file \'intact.fasta\'. Please, download it at ftp://ftp.ebi.ac.uk/pub/databases/intact/current/various/intact.fasta')
            sys.exit(10)

        sys.stderr.write("Parsing of the FASTA file %s\n" %FastaFile)
        fasta_file_fd = open(FastaFile,'r')
        intact_id_regex = re.compile(">INTACT:EBI-([0-9]+)\s\S+")
        ...
        for line in fasta_file_fd:
            ...
            if m:
                if len(sequence)>0:
                    self.intact2seq[intact_id] = sequence
                sequence = []
                intact_id = m.group(1)
            else:
        ...
        fasta_file_fd.close()
        sys.stderr.write("Parsing of the FASTA file finished\n")
...
#                continue
    ...
    def addXRefAttributesToExternalEntityObject(self, objPsi_MiXRef, psi_MiFormatted_object, flagIgnoreRefSecondary=False, attribute_class=ExternalEntityAttribute):
        if objPsi_MiXRef.refPrimary is not None:
            ...
            if dbNameConverted == 'IntAct':
                if dictFieldValue in self.intact2seq:
                    psi_MiFormatted_object.add_attribute(attribute_class(attribute_identifier = 'ProteinSequence', 
                                                                         value = ProteinSequence("".join(self.intact2seq[dictFieldValue])),
                                                                         type = 'unique') )
                else:
                    #print('IntAct protein {} does not have protein sequence'.format(dictFieldValue))
                    pass
    ...
    def decideDBReferenceSpecificConversions(self, objDBReference):
        ...
        if dbUpper in ("UNIPROTKB", "UNIPROT", "UNIPROT KNOWLEDGE BASE", "SWISSPROT", "TREMBL", "UNIPROT/SWISS-PROT", "UNIPROT/TREMBL"):
            ...
            dbNameConverted = "WormBaseSequenceName"
            dictFieldValue = id
            #self.checkOrInsertDBNamePrefix(dbNameConverted, id[:6])
        elif dbUpper == "PUBMED":
